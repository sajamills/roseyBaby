...
## Bar/nightlife keyword push (2026-08-25)

Ranked 10 user-supplied keywords by fit and real search-competition data (`WebSearch`, not guesswork), then built out the items with real content behind them:

- **Shipped**: retuned the existing `where-to-watch-msu-games` guide toward "game day bars starkville"/"bars near Mississippi State," added a companion blog post (`/blog/game-day-bar-near-mississippi-state`), and three new guides — `/starkville/downtown-starkville-bars`, `/starkville/cocktail-bars-starkville`, `/starkville/best-places-to-eat-starkville` — plus on-page links from `/beer-wall` and a cocktails blurb on `/menu`. Added `"BarOrPub"` to the sitewide `Restaurant`/`LocalBusiness` schema in `lib/seo.ts` to help Google associate the business with bar queries generally.
- **"Happy hour Starkville" — unblocked and shipped 2026-08-25.** The user provided the real weekly specials ($3 martinis Monday, $3 drafts Tuesday, half-off wine Wednesday, $3 bottle beer Thursday, $3 frozen drinks Friday, $2 mimosas Saturday; all day, dine-in only), so this got the full wing-night treatment: `/starkville/happy-hour-starkville` guide, `/blog/happy-hour-at-rosey-baby` post, a promo box on `/menu`, and a 5th homepage feature card (renamed the shared `.wing-night-promo` CSS class to `.menu-promo-callout` since it's now used by two different promos; widened `.feature-grid` from 4 to 5 columns).
- **"Birthday venue Starkville" — still skipped**, only implied generically under catering with no dedicated package. Needs the same thing happy hour needed: real terms from the user before writing anything.
- **Not building an owned page for**: "Nightlife in Starkville Mississippi" (intent mismatch — a family Cajun restaurant isn't a nightlife/club destination) and "Best bars in Starkville MS" (too saturated by aggregators and well-branded dedicated bars — Dave's Dark Horse Tavern, The Guest Room, Fountain Bar, Rick's Cafe — to realistically outrank with owned content). For both, the better lever is off-page: get Rosey Baby correctly categorized/listed on Tripadvisor's "Bars & Pubs" and Yelp's "Bars" category pages for Starkville, the same class of fix already logged for the Tripadvisor wing pricing above.
- "Cotton District bars Starkville" was ruled out earlier — Rosey Baby is on South Jackson Street near downtown, not the Cotton District (that's a different Crissey-family business, Coconuts, on University Drive). Targeting it would misrepresent the location.

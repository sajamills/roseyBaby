...
Still open (not attempted — needs real profiling, not guesswork):

- [Mobile performance] Largest Contentful Paint 4.7s mobile vs. 0.6s desktop. The gap suggests render-blocking or 3rd-party-script delay, not raw asset size — needs an actual LCP-breakdown/network-dependency-tree trace (Chrome DevTools Performance panel or a fresh Lighthouse run) to identify the real bottleneck rather than guessing.
- [Performance] "Improve image delivery" (9–160 KiB) and "Render-blocking requests" — not yet investigated.
...
- Long main-thread tasks (3–7, varies by run) — not yet profiled to find the specific culprit.
